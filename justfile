...
doctests:
    uv run --group doctests --all-extras xdoctest -m src/pysec2pri
...
format:
    ...
    # Use the version pinned in the "lint" dependency group, not uvx's latest,
    # so format/lint results are reproducible across machines and CI.
